...
function buildFile(file = {}, platform = {}, dictionary = {}) {
  ...
  const dirname = fullDestination.substring(
    0,
    fullDestination.lastIndexOf("/")
  );
  ...
  Object.keys(nameCollisionObj).forEach((propertyName) => {
    if (nameCollisionObj[propertyName].length > 1) {
      let collisions = nameCollisionObj[propertyName]
        .map((properties) => {
          return `${properties.path.join(".")}   ${properties.value}`;
        })
        .join("\n        ");
      GroupMessages.add(
  ...
  let filteredReferencesCount = GroupMessages.count(
  ...
  if (
    (nested || propertyNamesCollisionCount === 0) &&
    filteredReferencesCount === 0
  ) {
    console.log(`✔︎ ${fullDestination}`);
  } else {
    console.warn(`⚠️ ${fullDestination}`);
    ...
    if (filteredReferencesCount > 0) {
      ...
      let title = `While building '${destination}', filtered out token references were found; output may be unexpected. Here are the references that are used but not defined in the file`;
      let help = [
