  ...
  describe("reference", () => {
    describe("getReferences()", () => {
      ...
      it(`should work with object values`, () => {
        expect(
          dictionary.getReferences(properties.border.primary.value)
        ).toEqual(
      ...
      it(`should work with objects that have numbers`, () => {
        expect(
          dictionary.getReferences(properties.border.secondary.value)
        ).toEqual(expect.arrayContaining([{ value: "#f00" }]));
      });

      it(`should work with interpolated values`, () => {
        expect(
          dictionary.getReferences(properties.border.tertiary.value)
        ).toEqual(
          expect.arrayContaining([{ value: "2px" }, { value: "#f00" }])
        );
      });
