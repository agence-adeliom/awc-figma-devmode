...
describe("extend", () => {
  ...
  describe("includes", () => {
    it("should throw if include isnt an array", () => {
      expect(StyleDictionary.extend.bind(null, { include: {} })).toThrow(
    ...
    it("should properly glob paths", () => {
      var StyleDictionaryExtended = StyleDictionary.extend({
        include: helpers.getTokens(["./__tests__/__properties/*.json"]),
      });
      expect(typeof StyleDictionaryExtended.properties.size.padding.tiny).toBe(
        "object"
      );
    });
    ...
    it("should build the properties object if an include is given", () => {
      var StyleDictionaryExtended = StyleDictionary.extend({
        include: helpers.getTokens(["./__tests__/__properties/paddings.json"]),
      });
      var output = helpers.fileToJSON("./__tests__/__properties/paddings.json");
    ...
    it("should update properties if there are includes", () => {
      var StyleDictionaryExtended = StyleDictionary.extend({
        include: helpers.getTokens(["./__tests__/__configs/include.json"]),
      });
      expect(typeof StyleDictionaryExtended.properties.size.padding.tiny).toBe(
        "object"
      );
    ...
    it("should override existing properties if there are includes", () => {
      ...
      expect(StyleDictionaryExtended).toHaveProperty(
        "properties.size.padding.tiny.value",
        "3"
      );
    });
  });
  ...
  describe("source", () => {
    it("should throw if source isnt an array", () => {
      expect(StyleDictionary.extend.bind(null, { source: {} })).toThrow(
    ...
    it("should build the properties object if a source is given", () => {
      var StyleDictionaryExtended = StyleDictionary.extend({
        source: helpers.getTokens(["./__tests__/__properties/paddings.json"]),
      });
      var output = helpers.fileToJSON("./__tests__/__properties/paddings.json");
      traverseObj(output, (obj) => {
        if (obj.hasOwnProperty("value")) {
  ...
  it("should not throw a collision error if a source file collides with an include", () => {
    var StyleDictionaryExtended = StyleDictionary.extend({
      include: helpers.getTokens(["./__tests__/__properties/paddings.1.json"]),
      source: helpers.getTokens(["./__tests__/__properties/paddings.json"]),
      log: "error",
    });
    var output = helpers.fileToJSON("./__tests__/__properties/paddings.json");
  ...
  it("should throw a error if the collision is in source files and log is set to error", () => {
    expect(
      StyleDictionary.extend.bind(null, {
        source: helpers.getTokens([
          "./__tests__/__properties/paddings.1.json",
