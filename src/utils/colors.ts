export const rgbToHex = ({ r, g, b, a }: RGBA): string => {
  if (a !== 1) {
    return `rgba(${[r, g, b]
...
export const parseColor = (color: string) => {
  ...
  const floatRgbRegex = /^\{\s*r:\s*[\d\.]+,\s*g:\s*[\d\.]+,\s*b:\s*[\d\.]+(,\s*opacity:\s*[\d\.]+)?\s*\}$/;
...
export const hslToRgbFloat = (h: number, s: number, l: number) => {
